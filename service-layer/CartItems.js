...
const serviceUpdateCartItem = async (cartitemForm, cartitem_id) => {
  const response = await cartitemDAL.updateCartItemDAL(
    cartitemForm,
    cartitem_id,
  );
  return response;
...
const serviceClearUserCartItems = async (user_id) => {
  const response = await cartitemDAL.clearUserCartDAL(user_id);
  return response;
};
