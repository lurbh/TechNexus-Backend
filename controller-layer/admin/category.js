...
router.get("/add-category", async function (req, res) {
  const allCategories = (await serviceLayer.serviceGetCategories()).map(
...
router.post("/add-category", async function (req, res) {
  ...
  categoryForm.handle(req, {
    success: async function (form) {
      ...
      req.flash(
        "success_messages",
        `New Category ${category.get("category_name")} has been created`,
      );
...
router.post("/update-category/:category_id", async function (req, res) {
  ...
  categoryForm.handle(req, {
    success: async function (form) {
      if (form.data.parent_category_id === 0)
        delete form.data.parent_category_id;
      const category = await serviceLayer.serviceUpdateCategory(
        form,
        category_id,
      );
      req.flash(
        "success_messages",
        `Category ${category.get("category_name")} has been updated`,
      );
      res.redirect("/admin/categories");
    },
    empty: function (form) {
