...
router.get('/add-comment', async function(req,res){
    const allNewsArticles = (await newsService.serviceGetNewsArticles()).map( news => [ news.get('id'), news.get('title')]);
...
router.post('/add-comment', async function(req,res){
    ...
    commentForm.handle(req, {
        'success': async function(form) {
            ...
            req.flash("success_messages", `New Comment ${comment.get('comment_name')} has been created`)
...
router.post('/update-comment/:comment_id', async function(req,res){
    ...
    commentForm.handle(req, {
        'success': async function(form) {
            ...
            req.flash("success_messages", `Comment ${comment.get('comment_name')} has been updated`)
            res.redirect("/admin/comments");
        },
        'empty': function(form) {
            res.render('comments/create', {
