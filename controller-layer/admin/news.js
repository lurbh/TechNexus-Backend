...
router.post("/add-new-article", async function (req, res) {
  ...
  newsArticleForm.handle(req, {
    success: async function (form) {
      ...
      req.flash(
        "success_messages",
        `New NewsArticle ${newsArticle.get("newsArticle_name")} has been created`,
      );
...
router.post("/update-new-article/:newsArticle_id", async function (req, res) {
  ...
  newsArticleForm.handle(req, {
    success: async function (form) {
      if (form.data.parent_newsArticle_id === 0)
        delete form.data.parent_newsArticle_id;
      const newsArticle = await serviceLayer.serviceUpdateNewsArticle(
        form,
        newsArticle_id,
      );
      req.flash(
        "success_messages",
        `NewsArticle ${newsArticle.get("newsArticle_name")} has been updated`,
      );
      res.redirect("/admin/news");
    },
    empty: function (form) {
